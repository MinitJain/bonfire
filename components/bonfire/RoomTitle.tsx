...
interface RoomTitleProps {
  state: Pick<BonfireState, 'name' | 'initiator_name' | 'focus_duration' | 'short_duration' | 'long_duration'>
}

const minutes = (seconds: number) => Math.round(seconds / 60)

/**
 * The Bonfire's name and its configuration, quietly, at the top of the scene:
 *
 *   Deep Work            (or "Quiet Fox's fire" when unnamed)
...
export function RoomTitle({ state }: RoomTitleProps) {
  ...
  return (
    <div className="bf-room-title">
      <h1 className="bf-room-name">{bonfireTitle(state)}</h1>
      <p
